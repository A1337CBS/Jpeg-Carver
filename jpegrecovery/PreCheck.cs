...
namespace JpegRecovery
{
    class PreCheck
    {
        ...
        public void parseDht(FileStream fileStream, long dht_point=0)
        {
            ...
            //for (int i = 0; i < 4; i++)
            //{
            //    File.AppendAllText(path, string.Join(",", _dht[i].mincode) + Environment.NewLine);
            //}

            JsonSerializer serializer = new JsonSerializer();
            serializer.NullValueHandling = NullValueHandling.Ignore;
            Huffman.DHTStruct allDHTs = Huff2DHTStruct(_dht);
            allDHTs.id = allDHTs.GetHashCode();
            List<Huffman.DHTStruct> allDHTsList = new List<Huffman.DHTStruct>();
            allDHTsList.Add(allDHTs);

            using (StreamWriter sw = File.AppendText(@"Huffman.txt"))
            using (JsonWriter writer = new JsonTextWriter(sw))
            {
                serializer.Serialize(writer, allDHTsList.ToArray());
